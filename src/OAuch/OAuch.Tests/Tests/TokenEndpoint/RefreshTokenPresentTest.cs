...
namespace OAuch.Compliance.Tests.TokenEndpoint {
    ...
    public class RefreshTokenPresentTestImplementation : TestImplementation {
        ...
        public async override Task Run() {
            ...
            var provider = flows.CreateProviderWithStage<AddScope, Dictionary<string, string?>, Dictionary<string, string?>>(Context, (f, p) => p is ClientCredentialsTokenProvider);
            if (provider == null) {
                LogInfo("Could not find a working client credentials provider");
                Result.Outcome = TestOutcomes.Skipped;
                return;
            }
            ...
            if (token.RefreshToken != null) {
                LogInfo("The client credentials flow issues refresh tokens");
            ...
            LogInfo("The client credentials flow does not issue refresh tokens");
